...
describe("useResizablePanels", () => {
  ...
  it("restores sizes from localStorage on mount", async () => {
    ...
    const { result } = renderHook(() => useResizablePanels());
    // The mount effect schedules the localStorage read via queueMicrotask
    // so the setState doesn't run synchronously inside the effect body.
    // Drain microtasks + flush React effects before asserting.
