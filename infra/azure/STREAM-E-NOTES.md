...
> Design authority: `docs/history/specs/2026-06-12-azure-bicep-canonical-aionic-compute-design.md` (D4/D5/D8).
...
| `infra/azure/helm/templates/orchestrator-secretproviderclass.yaml` | Secrets Store CSI SecretProviderClass that pulls provider credentials (including opt-in `azure-foundry-api-key`) from Key Vault and syncs them to k8s Secret `reprolab-orchestrator-secrets`. Gated on `.Values.orchestrator.enabled`. |
