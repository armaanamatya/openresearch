...
#
# Creates four Secret Manager secret NAMES that the orchestrator pod reads at
...
#
# The four secrets exist in the same GCP project as the GKE cluster so the
...
resource "google_secret_manager_secret" "azure_foundry_api_key" {
  project   = var.project_id
  secret_id = "azure-foundry-api-key"
