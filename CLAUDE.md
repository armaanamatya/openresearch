...
- **`detect_environment` runs locally, not on RunPod**: `environment_spec.json::compatibility_notes` always says "Generated for pytorch==X on CPU." on a CPU-only dev machine. The root must NOT use this note to conclude the experiment is CPU-only — `detect_environment` derives framework/package specs from the paper, but the actual execution environment is always the RunPod pod with CUDA. Known instance: BUG-NEW-047 (`compute_scope_invalid` false warning when root passes a CPU-only string for `compute_scope`). Generated `train.py` should always include runtime GPU detection (`torch.cuda.is_available()`) rather than relying on `compute_scope` metadata.
- **`paper_grounding_failed` false positives on dict-repr dataset strings**: the root sometimes returns `datasets` as a list of dict objects (e.g. `{"name": "CIFAR-10", ...}`). `PaperClaimMap` stringifies these to `str(dict)` which the grounding validator correctly rejects as not found in the paper text (BUG-NEW-046). This is advisory-only and the run continues, but produces noisy `run_warning` events. Upstream fix: root prompt should specify `datasets` must be plain name strings.
