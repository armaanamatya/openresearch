...
- **Rubric verification + self-improvement** (`rubric_verifier_enabled`): a `rubric-verifier` agent scores against a PaperBench-style weighted rubric at Gate 2 (`baseline_verification`) and Gate 3 (`improved_verification`). The canonical rubric is resolved once per run (vendored bundle's `rubric.json` or LLM-generated) and stored on `PipelineState.rubric_spec` so the two checkpoints stay comparable. When `baseline_verification` is below `rubric_target_score`, the orchestrator drives improvement-selection + Gate 3 entirely through `_run_improvement_reiteration_loop` (seeded from baseline on iteration 1, then keyed on improved), capped by `rubric_max_improvement_iterations` total rounds. Any supervisor non-pass at Gate 2 or Gate 3 — including `partial_reproduction` — is terminal: the run halts and `_finalize_partial` writes `final_report.{json,md}`. With `rubric_verifier_enabled=False`, a single unconditional improvement round runs so `generate_research_map` has data. Fail-closed: a verifier error degrades to the heuristic rubric. See `docs/design/option-d-q1q2-refactor.md`.
