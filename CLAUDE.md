...
`REPROLAB_CONTEXT_MAP=on` (default off) enables a free, deterministic intra-run orientation cache. A one-line write hook in `backend/agents/rlm/binding.py` (the primitive success chokepoint, beside `_emit_supplemental`) unions the structured outputs of `understand_section` / `extract_hyperparameters` / `detect_environment` into `runs/<id>/rlm_state/context_map.json` — keyed `primitive:field`, **union-per-field** (NOT latest-wins) so a multi-section paper (SDAR's 3 model sizes + 3 environments) accumulates instead of clobbering. The root reads it via the pure-I/O `read_context_map()` primitive and — **only when the flag is on** — is told (system prompt, appended by `build_system_prompt`) to consult it before re-deriving a fact via `rlm_query`/`llm_query`. Bounded: ≤40 fields, ≤8 values/field, ≤8 KB (byte ceiling enforced **incrementally** — keeps valuable-first entries, drops only the overflow value, not the whole write; 8 KB not 1.5 KB because the map is a `read_context_map()` return value, never prompt-injected, so a measured ~3.6 KB full SDAR pass fits with headroom instead of being truncated); thread-safe (module lock + atomic `tmp`+`os.replace`); fail-soft on every path. The map is a **navigation aid, never a report source** — the Phase 3 evidence gate (`REPROLAB_EVIDENCE_GATE`) remains the report backstop. **Off-state (default):** the write hook no-ops, `read_context_map()` returns empty, and the prompt instruction is omitted (the root is never told to call it). The only residue is `read_context_map` remaining in the auto tool inventory with a **declarative, self-disclosing description** (no imperative to call it — the "call it before re-deriving" instruction lives only in the gated prompt section): callable, returns empty, ~50 cached-prefix tokens, no LLM cost, no instructed calls. It is NOT redundant with `primitive_cache.py`: the cache returns the same per-slice result, while the map aggregates *across* slices (the cache structurally cannot). Module: `backend/agents/rlm/context_map.py`; design `docs/superpowers/specs/2026-05-30-intra-run-context-map-design.md`. **A/B before making default:** `=on` vs `=off` across ≥3 paired SDAR runs — compare iteration count + `rlm_query`/`llm_query` call counts (expect a smaller delta than PEEK's headline, since a cache already exists); `rubric.overall_score` must not regress. **Rollback:** unset the flag.

### Per-paper negative lessons (MUSE-lite, spec 2026-05-30)
`REPROLAB_NEGATIVE_LESSONS=1` (default off) turns on a cross-run, per-paper failure memory. A post-run hook in `run.py::_finalize` (`backend/agents/rlm/lesson_distiller.py::mine_lessons`) reads this run's `experiment_runs.jsonl`, keeps only **agent-correctable** `failure_class` rows (`missing_module`, `requirements_not_found`, `syntax_error`, `scope_shape_violation`, `contract_violation`, `dockerfile_invalid`, …; environmental/transient classes are excluded), and upserts them into `runs/_lessons/<arxiv_id>.json` (gitignored via `/runs/`). On the next run of the same `arxiv_id`, `baseline_implementation.py::_negative_lessons_block` injects the **active** lessons into the implementer guidance. Three guardrails keep it honest: (1) **recurrence-gated promotion** — a lesson is `candidate` (not injected) until `occurrences >= 2`, except structurally-deterministic classes (`dockerfile_invalid`) which promote at 1; (2) **classifier-sourced `suggested_fix`** — re-derived from `failure_classifier.suggested_fix_for_class()`, never the agent-authored `experiment_runs.jsonl::suggested_fix` field (no prose side-door); (3) **opportunity-aware retirement** — staleness ages a lesson only when its gating phase was reached (`EXPERIMENT_ATTEMPTED` = ≥1 record; `EXPERIMENT_SUCCEEDED` = a `success==true`+non-empty-metrics row, the same predicate as the evidence gate), derived from `experiment_runs.jsonl` only (no Dockerfile-exists heuristic), retired at `staleness >= 3`. Capped (≤5 injected, fix ≤200 chars), advisory only (the evidence gate + rubric remain the correctness backstops). Mining needs `ctx.arxiv_id`; a PDF upload with no id does not participate. **A/B before default:** `=1` vs `=0` across ≥3 paired SDAR runs — does a class-tagged lesson cut recurrence of its `failure_class` without lowering `rubric.overall_score`? **Rollback:** unset the flag.
...
**Local Docker daemon is a prerequisite for every sandbox except `local` — including `runpod` (the repo default).** `build_environment` (`primitives.py:1090`) short-circuits to a no-op ONLY when `ctx.sandbox_mode == "local"` (`:1113`); for `docker`, `runpod`, `auto`, and any unknown mode it runs a real local `docker build` and raises `SandboxRuntimeError(backend_unavailable)` if no daemon is reachable. Under `runpod` the locally-built image is **never used on the pod** (the pod boots `REPROLAB_RUNPOD_IMAGE` and runs over SSH in a per-run venv — `runpod_backend.py`), so the local build is currently wasted work that still hard-requires Docker; **flagged rough edge (2026-05-30)** — a future change could short-circuit `build_environment` under `runpod` too. Until then: keep OrbStack/Docker up for RunPod runs, or use `--sandbox local`. Since 2026-05-30 `start.sh` preflight-checks `docker info` whenever the sandbox is not `local`. The full workflow + a sandbox×prerequisite matrix + troubleshooting table live in `docs/runbooks/running-the-project.md`. A hollow `partial` with `SDK success-with-no-text` is an **auth/SDK** failure (root/sub-agent credentials), NOT Docker — read the `/lab` detail-panel blockers to tell them apart.

### Local multi-GPU sandbox (docker-free, spec 2026-05-29)
For a host with local NVIDIA GPUs but **no docker access and no RunPod** (e.g. the 8×A5000 box), the `local` sandbox runs experiments as host subprocesses with no image build. Three root fixes made `--sandbox local` actually work (they were latent-broken before): `_backend_for_sandbox_mode` now routes `SandboxMode.local` → `LocalProcessBackend` (was silently falling back to docker); `build_environment` short-circuits to a no-op success under `local` (was hardwired to `docker build`); and `SandboxConfig.gpu_device_ids` pins GPUs (`LocalProcessBackend` → `CUDA_VISIBLE_DEVICES`, `LocalDockerBackend` → `DeviceRequest(device_ids=...)` instead of the old `count=-1` "grab all GPUs" bug). `RunContext.gpu_device_ids` is populated from `REPROLAB_GPU_DEVICE_IDS`.

Dynamic GPU scheduling lives in `backend/services/runtime/local_gpu_allocator.py` (`discover_gpus`/`free_devices`/`LocalGpuAllocator`): nvidia-smi discovery, **excludes GPUs another user holds** (used-mem ≥ threshold OR any compute proc), file-backed `fcntl`-locked crash-safe leasing by GPU UUID, stale-lease reclaim by PID liveness. Nothing is hardcoded — capacity derives from currently-free GPUs.

The batch scheduler is `scripts/batch_reproduce.py` (run with the project venv: `.venv/bin/python scripts/batch_reproduce.py <paper>... --gpus-per-run 4|auto --model claude-oauth`). It leases disjoint GPU sets, creates a **per-run venv** (`runs/<id>/.venv`, isolated paper deps) with a **shared `HF_HOME`** weight cache, sets per-run torch/triton/xdg/mpl caches, launches `python -m backend.cli reproduce ... --project-id <id>`, monitors, releases leases (SIGINT-safe). `--gpus-per-run auto` gives one run all currently-free cards (heavy Qwen). For `local`, `build_environment` is a no-op and the per-run `requirements.txt` is auto-installed into the venv (mirrors the runpod bootstrap). **Setup note:** this host needs Python 3.12+ (repo `requires-python>=3.11`); provision with `uv venv --python 3.12 .venv` (a fresh clone ships no venv). Known repo gotcha: `rlms==0.1.1` depends on `pytest>=9.0.2`, which conflicts with `requirements-dev.txt`'s `pytest>=8,<9` — install `backend/requirements.txt` alone (rlms pulls a working pytest 9).
