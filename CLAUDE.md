...
## Sandboxes
`REPROLAB_DEFAULT_SANDBOX` selects the execution backend: `local`, `docker` (network/memory/CPU controlled), `runpod` (remote GPU pods, requires `REPROLAB_RUNPOD_API_KEY` and `REPROLAB_RUNPOD_SSH_KEY_PATH`), `azure` (Azure VMs via the Compute SDK, requires `REPROLAB_AZURE_SUBSCRIPTION_ID` and `REPROLAB_AZURE_SSH_KEY_PATH`), or `brev` (NVIDIA Brev instances). `start.sh` runs `scripts/runpod_check.sh` or `scripts/azure_check.sh` as a preflight depending on the selected sandbox; bypass with `START_SKIP_PREFLIGHT=1`. `START_FULL_SMOKE=1` (RunPod) or `START_FULL_AZURE_SMOKE=1` (Azure) boots a real instance for end-to-end verification — **this costs money** (cents-scale on RTX 4090; <\$0.01 for the Azure CPU smoke).
...
### Azure compute sandbox (2026-05-27)
`--sandbox azure` provisions an Azure VM via the Compute SDK (`azure-mgmt-compute` + `azure-identity`; no `az` CLI shellout). Auth resolves via `DefaultAzureCredential`, so `az login`, service-principal env vars (`AZURE_CLIENT_ID`/`_SECRET`/`_TENANT_ID`), or managed identity all work. Provisions: ephemeral or named RG → vnet/subnet → NSG (open 22 inbound) → public IP → NIC → VM. Tear-down is `asyncio.shield`-wrapped + atexit-registered; ephemeral RGs cascade-delete in one ARM call. Defaults: `Standard_NC6s_v3` in `eastus`. The shared `pod_sweep_scheduler` now sweeps stale `reprolab=true`-tagged Azure VMs on the same interval as RunPod pods. Every ARM call writes to `runs/<id>/azure_resource_log.jsonl`; boot console writes to `runs/<id>/azure_provisioning.log`. Cost-bounded by the existing `REPROLAB_MAX_RUN_GPU_USD` / `REPROLAB_MAX_GPU_USD_PER_HOUR` gates. **Independent of the Azure OpenAI LLM provider** (`azure-gpt-4o` in `models.py`) — having one configured does NOT auto-enable the other; they share only the prefix. See `docs/runbooks/2026-05-27-azure-sandbox.md` for the operator runbook.

## Forced-iteration policy (Lane H, spec 2026-05-24)
When the root model calls `FINAL_VAR` but the latest `verify_against_rubric` returned `overall_score < target_score` AND `iteration_count < REPROLAB_MIN_RUBRIC_ITERATIONS` (default 2), the orchestrator REFUSES the `FINAL_VAR` and forces the root-loop to continue. Mechanically: `backend/agents/rlm/forced_iteration.py` patches `rlm.environments.local_repl.LocalREPL._final_var` once at import time; a per-run `ForcedIterationPolicy` is pushed onto a thread-local stack via the `forced_iteration_policy` context manager around `rlm.completion`. Each refusal emits a `run_warning` event with `code="forced_iteration"` and a message naming the score/target/iteration numbers and the concrete next step (propose_improvements + implement_baseline + run_experiment). Wall-clock takes precedence: when `ctx.remaining_s() <= 60s` the policy bypasses and the partial report ships. `REPROLAB_MIN_RUBRIC_ITERATIONS=0` disables the policy entirely. Paired with Lane G (`backend/agents/rlm/rubric_guard.py` — the agent-written train.py calls `assert_metrics_schema(...)` at end-of-script; a missing key / artifact raises `RubricGuardFailure` whose JSON-shaped message becomes the next iteration's `repair_context`).
