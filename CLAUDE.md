...
- **`GET /v1/models` 200 is NOT proof the OpenAI key works.** Listing models never consumes quota, so a models-list connectivity check returns `HTTP 200` even on a key with zero balance; only a real `chat.completions` call proves it. An out-of-quota key surfaces as `429 {code: insufficient_quota}` and a `--model gpt-5` run wedges forever in `generate_rubric_tree` (backoff-retrying 429, burning wall-clock, never booting a pod). When OpenAI is dry, the gpt-5 root **and** the `REPROLAB_ACCELERATOR=endpoint` gpt-5-mini route are both dead — fall back to `--model claude-oauth` + `REPROLAB_ACCELERATOR=off` (subscription on both surfaces). Narrative: `docs/runbooks/2026-05-23-sdar-baseline-handoff.md` (2026-05-31 attempt).
...
- **GEPA per-run prompt optimization** — `REPROLAB_GEPA_OPTIMIZATION` (settings `gepa_optimization`) ∈ `off` (**default**) / `on` / `plan-only` / `baseline-only` / `improve-only`. When on, `wrap_primitive()` fires a bounded GEPA (Genetic-Pareto, `gepa.optimize()`) mini-loop *before* each `plan_reproduction` / `implement_baseline` / `propose_improvements` call (`gepa_pre_call`), picks the best system prompt for that call, injects it, then `gepa_post_call` appends the real result to `ctx.gepa_example_buffer` (persisted `runs/<id>/gepa_examples.jsonl`) so the next call's trainset is warmer. **Two injection paths:** Path A monkey-patches `ctx.llm_client.complete` (plan/improve); Path B threads `system_prompt_override` through `run_with_sdk`→`collect_agent_text`→`spec.instructions` for `implement_baseline` (its claude-agent-sdk sub-agent never touches `llm_client.complete`). Bounds: max metric calls `10/5/10` (plan/baseline/improve), timeouts `180/90/180`s (raised from 60/30/60 in `a1377d0` — BUG-NEW-048 fix: claude-oauth seed evaluation takes ~2 min, prior 60s was exhausted before any proposals), reflection model `gepa_reflection_model` (default `claude-oauth` — uses Claude Code subscription, no API key needed; set to an `openai/gpt-*` or `anthropic/claude-*` litellm string to use a paid key instead). **Fail-soft:** `gepa`/`gepa-viz` are in `backend/requirements.txt`; `run_gepa_mini` never raises — any error returns the seed prompt, so any import error is also a no-op. Re-entrancy guard `ctx.gepa_optimization_active` blocks GEPA-inside-GEPA. Live viz: gepa-viz on `gepa_viz_port` (default 5151), proxied via `frontend/src/app/api/gepa-viz/[...path]/route.ts`. SSE events `gepa_phase_start`/`gepa_phase_complete`/`gepa_candidate_proposed`/`gepa_candidate_accepted`/`gepa_candidate_rejected`. Code: `backend/agents/gepa/`. Spec `2026-05-31-gepa-per-run-integration-design.md`.
...
- `rlm_state/` — per-iteration checkpoints; resume-safe; also holds `anytime_score.json` (rubric score written after first successful experiment, read by `_finalize` + watchdog as fallback when root times out before calling `verify_against_rubric`)
...
4. SSE event types: RLM emits `repl_iteration`, `primitive_call`, `sub_rlm_spawned`, `sub_rlm_complete`, `run_complete`, `candidate_proposed`, `candidate_outcome`, `rubric_score`, `user_message`, `user_message_response`, `run_warning`, `iteration_heartbeat`; RDR adds `rdr_*` plus `cluster_started`, `cluster_artifact_emitted`, `cluster_scored`, `repair_dispatched`; the GEPA flag (when on) adds `gepa_phase_start`, `gepa_phase_complete`, `gepa_candidate_proposed`, `gepa_candidate_accepted`, `gepa_candidate_rejected`.
...
- GEPA optimization (feature-flagged, off by default): `backend/agents/gepa/` (`hooks.py` = pre/post-call, `optimizer.py` = `run_gepa_mini`, `callback.py` = `SSEGepaCallback`); wired into `backend/agents/rlm/binding.py::wrap_primitive`; viz proxy `frontend/src/app/api/gepa-viz/[...path]/route.ts`.
...
**Local Docker daemon is a prerequisite for `--sandbox docker` and `auto`/unknown, but NOT for `local` or `runpod`.** `build_environment` (`primitives.py:1123`) short-circuits to a no-op when `ctx.sandbox_mode` is `local` (line 1146) **or `runpod` (line 1163)** — for `runpod` it returns before any `docker build` because the pod boots `REPROLAB_RUNPOD_IMAGE` and runs over SSH in a per-run venv (`runpod_backend.py`), so a local image would be unused. The real `docker build` + `SandboxRuntimeError(backend_unavailable)` path (~line 1297) is reached only for `docker`/`auto`/unknown. The runpod short-circuit landed in **`875995c`** (the "rough edge flagged 2026-05-30" of a wasted local build under runpod — now resolved); `82e9806` also normalizes a hallucinated `runpod/` FROM line for the unreachable docker-path case. `start.sh`'s `docker info` preflight warns when the default sandbox is `docker`/`auto` (not `local`, not `runpod`). Keep OrbStack/Docker up only for `--sandbox docker`/`auto`. Full workflow + sandbox×prerequisite matrix + troubleshooting: `docs/runbooks/running-the-project.md`. A hollow `partial` with `SDK success-with-no-text` is an **auth/SDK** failure (credentials), NOT Docker — read the `/lab` detail-panel blockers to tell them apart.

**RunPod default image is `cuda-devel`** (`runpod/pytorch:2.1.0-py3.10-cuda11.8.0-devel-ubuntu22.04`, torch 2.1.0) — set in `runpod_backend.py::DEFAULT_RUNPOD_IMAGE`. The lighter `cuda-runtime` (~4 GB) was tried but **reverted** (`88c45b0`): it ships no dev headers, so `pip install` of `bitsandbytes` / `flash-attn` / `deepspeed` silently failed and surfaced downstream as `ModuleNotFoundError: transformers` (debug history: `docs/runbooks/2026-05-23-sdar-baseline-handoff.md`). Keep `devel` unless you can prove the paper's deps are all pre-built wheels; override with `REPROLAB_RUNPOD_IMAGE=...-runtime-...` to save the larger pull at your own risk.

### Local multi-GPU sandbox (docker-free, spec 2026-05-29)
For a host with local NVIDIA GPUs but no docker/RunPod (e.g. 8×A5000): the `local` sandbox runs experiments as host subprocesses with no image build. `SandboxMode.local` → `LocalProcessBackend`; `build_environment` is a no-op success under `local`; `SandboxConfig.gpu_device_ids` pins GPUs (`LocalProcessBackend`→`CUDA_VISIBLE_DEVICES`, `LocalDockerBackend`→`DeviceRequest(device_ids=...)`). `RunContext.gpu_device_ids` from `REPROLAB_GPU_DEVICE_IDS`.
...
- **Forced-iteration policy** (`forced_iteration.py`, patches `LocalREPL._final_var` at import; per-run `ForcedIterationPolicy` via context manager around `rlm.completion`): when root calls `FINAL_VAR` but the latest `verify_against_rubric` score is `< target_score` **or `None`** AND `iteration_count < REPROLAB_MIN_RUBRIC_ITERATIONS` (default 2), refuses `FINAL_VAR` and continues the loop (emits `run_warning` `code="forced_iteration"` naming score/target/iteration + next step). Wall-clock wins: bypassed when `ctx.remaining_s() <= 60`. `REPROLAB_MIN_RUBRIC_ITERATIONS=0` disables. Paired with Lane G `rubric_guard.py` (`assert_metrics_schema` → `RubricGuardFailure` → next-iteration `repair_context`). Spec `2026-05-28-...` (BUG-LR-013).
...
- **`detect_environment` runs locally, not on RunPod**: `environment_spec.json::compatibility_notes` says "Generated for pytorch==X on **local CPU dev machine**. NOTE: this spec describes the LOCAL environment … the RunPod execution environment provides CUDA GPUs." The root must NOT use this note to conclude experiments run CPU-only — fixed in `a1377d0` (BUG-NEW-047). Generated `train.py` should always include runtime GPU detection (`torch.cuda.is_available()`) as the authoritative source.
- **`paper_grounding_failed` false positives on dict-repr dataset strings**: fixed in `a1377d0` (BUG-NEW-046). `paper_grounding.py::assert_paper_grounded` now extracts the `name` field from dict-repr strings before the grounding check, so `{"name": "CIFAR-10", ...}` is correctly matched against paper text. Root prompt should still use plain name strings for `datasets`.
- **Anytime fallback scoring**: after the first successful `run_experiment` with `remaining_s ≤ 1800` (or no deadline), `_try_anytime_score()` in `primitives.py` fires a blocking `verify_against_rubric` call and writes the result atomically to `rlm_state/anytime_score.json`. Both `_finalize` and the wall-clock watchdog read this file as a fallback when the final report's rubric score is `None` — preventing 0.0 verdicts on runs that timed out before the root called `verify_against_rubric`. Implemented `33b90c1`.
