...
def test_fatal_finalize_writes_failed_status_and_failed_report(make_context, tmp_path):
    ...
    # run hit a credit wall, it never really executed). Per the 2026-05-30 #1
    # tightening + self-attest escape closure, success==False does NOT license a
    # partial/reproduced VERDICT — so the report verdict is now "failed", even
    # though the partial numbers are preserved in baseline_metrics for the record.
    ctx = make_context(tmp_path)
