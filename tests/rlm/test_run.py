...
class TestResolveCustomTools:
    ...
    def test_uses_real_binding_when_present(self, tmp_path, make_context, monkeypatch):
        ...
        ctx = make_context(tmp_path)
        tools, label = _resolve_custom_tools(ctx)
        assert len(tools) == 16  # 9 original + record_candidate_outcome (Task 13) + check_user_messages + respond_to_user + heartbeat + recommend_next_tool + resolve_gpu_requirements + read_context_map
