...
def _assess(attempt_n, score, failure_class=None, gpu_usd=1.0):
    report = SimpleNamespace(score=score) if score is not None else None
    return SimpleNamespace(
        attempt_n=attempt_n,
        final_report=report,
        failure_class=failure_class,
        cost=SimpleNamespace(gpu_usd=gpu_usd),
    )
...
def test_off_does_not_read_attempt_cost(monkeypatch):
    class _UnreadableCost:
        @property
        def gpu_usd(self):
            raise AssertionError("OFF path must not inspect GPU cost")

    monkeypatch.delenv("OPENRESEARCH_SCHEDULER_TREE", raising=False)
    result = {"kind": "CONTINUE", "rule": "x"}
    assessment = _assess(1, 0.9)
    assessment.cost = _UnreadableCost()
    _maybe_attach_asha_advisory(result, [assessment], 0, max_gpu_usd=10.0)
...
def test_width_meter_uses_observed_per_attempt_gpu_cost(monkeypatch):
    # A real 3.0-$ attempt bounds k at 2.  The old uniform 1.0-$ estimate
    # would have promoted all four, so this guards against reintroducing it.
    monkeypatch.setenv("OPENRESEARCH_SCHEDULER_TREE", "1")
    result = {"kind": "CONTINUE"}
    cohort = [
        _assess(1, 0.9, gpu_usd=3.0),
        _assess(2, 0.7, gpu_usd=1.0),
        _assess(3, 0.5, gpu_usd=0.5),
        _assess(4, 0.3, gpu_usd=0.5),
    ]
    _maybe_attach_asha_advisory(
        result, cohort, 0,
        max_gpu_usd=10.0, gpu_usd_spent=4.0, max_gpu_count=100,
    )
    actions = [d["action"] for d in result["asha_advisory"]["decisions"]]
    assert actions.count("promote") == 2  # 6.0 // max(3.0, 1.0, 0.5) == 2
    assert actions.count("freeze") == 2


def test_malformed_attempt_costs_degrade_to_adapter_zero(monkeypatch):
    class _UnreadableCost:
        @property
        def gpu_usd(self):
            raise RuntimeError("corrupt cost artifact")

    monkeypatch.setenv("OPENRESEARCH_SCHEDULER_TREE", "1")
    captured = {}

    def _capture(_assessments, _config, *, gpu_usd_by_attempt=None):
        captured["costs"] = gpu_usd_by_attempt
        return []
    ...
    cohort = [
        _assess(1, 0.9, gpu_usd=2.0),
        _assess(2, 0.7, gpu_usd=-1.0),
        _assess(3, 0.5, gpu_usd=float("nan")),
        _assess(4, 0.3, gpu_usd=float("inf")),
        _assess(5, 0.1, gpu_usd="1.0"),
        _assess(6, 0.0, gpu_usd=True),
    ]
    cohort.append(_assess(7, 0.0, gpu_usd=0.0))
    cohort[-1].cost = _UnreadableCost()
    cohort.append(_assess(8, 0.0, gpu_usd=0.0))
    cohort.append(_assess(9, 0.0, gpu_usd=0.0))
    cohort[-1].cost = None
    result = {"kind": "CONTINUE", "rule": "policy_rule"}
    _maybe_attach_asha_advisory(
        result, cohort, 0,
        max_gpu_usd=10.0, gpu_usd_spent=2.0, max_gpu_count=8,
    )
    assert captured["costs"] == {1: 2.0, 8: 0.0}
    assert result["kind"] == "CONTINUE"
    assert result["rule"] == "policy_rule"
