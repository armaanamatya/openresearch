...
*local* `docker build` only for `--sandbox docker` and `auto`/unknown
(`LocalDockerBackend`). Since `875995c` it short-circuits to a no-op for **both
`--sandbox local` and `--sandbox runpod`** (the repo default) — under `runpod` the
experiment executes on a remote GPU pod that boots its own image over SSH, so **no
local Docker daemon is required**. A down daemon only breaks `docker`/`auto` runs
(`backend_unavailable` at `build_environment`). The full sandbox×prerequisite
...
## GEPA per-run prompt optimization (spec 2026-05-31, feature-flagged off by default)

`REPROLAB_GEPA_OPTIMIZATION` (default `off`; also `on` / `plan-only` / `baseline-only` / `improve-only`) wires GEPA (Genetic-Pareto reflective prompt optimization, the `gepa` PyPI package) into the primitive call path. Inside `wrap_primitive()` (`backend/agents/rlm/binding.py`), `gepa_pre_call` fires before each `plan_reproduction` / `implement_baseline` / `propose_improvements` call: it runs a bounded `gepa.optimize()` mini-loop (`run_gepa_mini` in `backend/agents/gepa/optimizer.py`) — capped at 10/5/10 metric calls and 60/30/60 s respectively, using the `gepa_reflection_model` (default `openai/gpt-4o`) — over a trainset seeded from historical examples plus the running `ctx.gepa_example_buffer`. The winning system prompt is stored in `ctx.gepa_prompt_overrides[name]` and injected for that one call; `gepa_post_call` then appends the real call result to the buffer (persisted to `runs/<id>/gepa_examples.jsonl`) so later calls in the same run optimize against warmer data.

Two injection paths exist because the primitives reach their LLM differently. **Path A** monkey-patches `ctx.llm_client.complete` for `plan_reproduction` and `propose_improvements` (which call the client directly). **Path B** serves `implement_baseline`, whose work runs in a claude-agent-sdk sub-agent that never touches `llm_client.complete`: a `system_prompt_override` kwarg is threaded from `ctx` through `run_with_sdk` → `collect_agent_text`, where `dataclasses.replace` swaps `spec.instructions` for the optimized prompt (Path A explicitly skips `implement_baseline` to avoid double-injection).

The subsystem is fail-soft and dormant by default: `gepa`/`gepa-viz` are not in `backend/requirements.txt` (installed separately), and `run_gepa_mini` catches `ImportError` and every other exception, returning the seed prompt — so an off flag and a missing package are both exact no-ops. A re-entrancy guard (`ctx.gepa_optimization_active`) prevents GEPA-inside-GEPA recursion. Optimization progress streams as `gepa_phase_start` / `gepa_phase_complete` / `gepa_candidate_proposed` / `gepa_candidate_accepted` / `gepa_candidate_rejected` SSE events (built in `sse_bridge.py`, emitted by `SSEGepaCallback`); the gepa-viz live UI (default port `gepa_viz_port=5151`) is reachable through the Next.js proxy `frontend/src/app/api/gepa-viz/[...path]/route.ts`. Code: `backend/agents/gepa/`; tests `tests/gepa/`.
...
- `docs/superpowers/specs/2026-05-31-gepa-per-run-integration-design.md` —
  per-run GEPA prompt optimization (feature-flagged off by default).
