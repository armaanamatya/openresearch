...
def build_environment(env_spec: dict, *, ctx: "RunContext") -> dict:
    ...
    # BUG-NEW-046: normalize runpod/ FROM line.  The root model sometimes
    # hallucinate a non-existent runpod image tag (e.g. runpod/pytorch:1.12.1).
    # When sandbox_mode is "runpod", replace any runpod/ FROM reference with
    # the settings-configured image to prevent manifest-not-found failures.
    if _sb_key == "runpod":
