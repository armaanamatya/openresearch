...
def build_anthropic_foundry_client(backend_kwargs: dict[str, Any]):
    ...
    # Fail closed on missing credentials, mirroring the two sibling paths
    # (run.py::_resolve_agent_runtime executor branch + grader_transport's
    # anthropic-foundry branch). Without this an empty base_url/api_key builds an
    # anthropic.Anthropic that only fails deep in a request with an opaque error.
    if not (base_url and api_key):
        raise ValueError(
            "anthropic-foundry root requires AZURE_FOUNDRY_ENDPOINT (or "
            "AZURE_FOUNDRY_ANTHROPIC_ENDPOINT) + AZURE_FOUNDRY_API_KEY"
        )
