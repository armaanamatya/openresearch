    ...
    if bib_file.exists():
        cmd.extend(
            [
                ...
                str(bib_file),
                '--csl',
                f'{citation_style}.csl' if not citation_style.endswith('.csl') else citation_style,
            ]
        )
...
def main():
    ...
    success = generate_pdf(
        markdown_file, output_pdf, citation_style=citation_style, toc=toc, number_sections=number_sections
    )
