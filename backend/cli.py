...
def cmd_reproduce(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "project_id", None):
        # ISSUE-1 (2026-05-30): the override may only *mirror* the source-derived
        # id. The REST API spawns the CLI with --project-id set to the SAME
        # deterministic id (project_id_for(source)) so the CLI writes to the dir
        # the API watches — that is a safe no-op. An ARBITRARY value (e.g. the old
        # runbook's `sdar_<ts>`) repoints project_id AFTER register_project keyed
        # the event-store aggregate by the source-derived id, so the very next
        # fetch_paper(project_id=...) misses the aggregate -> UnknownProject and the
        # run dies at ingest. Reject the mismatch with an actionable message rather
        # than failing opaquely three steps later.
