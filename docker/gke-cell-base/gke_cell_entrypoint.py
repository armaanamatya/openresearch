"""GKE Job in-container entrypoint for a single SDAR training cell.

Flow
----
1. Pull project code from Google Cloud Storage (``runs/<run_id>/code/``).
2. Set HF_HOME / TRANSFORMERS_CACHE / PIP_CACHE_DIR under the cache mount
...
3. ``pip install`` project requirements through that cache (delta only; wheels
   already cached on the shared volume make this ~instant after the first cell).
...
6. Push ``metrics.json`` + per-attempt logs + ``status.json`` to GCS.
...
--------------------
All business logic lives in PURE functions (no google-cloud import at module top):
  plan_attempts(max_oom_retries)          -> list[dict]
  is_oom(stderr)                          -> bool
  classify_outcome(exit_code, stderr, ...) -> tuple[int, str]
  build_sentinel(...)                     -> dict

GCS I/O is behind an injectable ``client`` param; when ``client is None`` the
real ``google.cloud.storage`` is imported lazily.  Tests pass a
``FakeBucketClient`` and run without any google-cloud package or GPU.
"""
...
# ---------------------------------------------------------------------------
# Pure functions (no google-cloud import, no subprocess — fully unit-testable)
# ---------------------------------------------------------------------------
...
def build_sentinel(
    *,
    ...
    """Build the status.json payload uploaded to GCS.
...
# ---------------------------------------------------------------------------
# GCS helpers (injectable client; lazy google-cloud import)
# ---------------------------------------------------------------------------

def _gcs_bucket_client(
    *,
    bucket_name: str,
    project: str | None,
    client: Any | None,
) -> Any:
    """Return a GCS Bucket handle, creating one lazily when *client* is None.

    The lazy creation imports ``google.cloud.storage`` only when needed — so
    tests that inject a fake client never touch the google-cloud package.  The
    project is inferred from Application Default Credentials (Workload Identity
    inside the GKE pod) when *project* is None; the optional ``project``
    argument is forwarded only when explicitly supplied.

    Args:
        bucket_name: GCS bucket name (the artifact bus).
        project:     GCP project ID, or None to infer from ADC.
        client:      Pre-built Bucket-like client, or None to construct one.

    Returns:
        A GCS Bucket handle (real or injected fake).
    """
...
def upload_bytes_to_gcs(
    ...
    *,
    ...
    """Upload raw bytes to a single GCS blob, overwriting if it exists.

    Args:
        data:        Raw bytes to upload.
        blob_name:   Destination blob name (path within the bucket).
        bucket_name: GCS bucket name.
        project:     GCP project ID (or None to infer from ADC).
        client:      Injectable Bucket-like client (None = build lazily).
    """
    bucket_client = _gcs_bucket_client(
...
def download_prefix_to_dir(
    ...
    *,
    ...
    downloaded: list[str] = []
    for blob in bucket_client.list_blobs(prefix=blob_prefix):
        name: str = blob.name
        ...
        # (gcs_blob._validate_blob_name) already rejects these; the download
        ...
        data: bytes = bucket_client.blob(name).download_as_bytes()
...
def upload_file_to_gcs(
    ...
    *,
    ...
    """Upload a local file to a single GCS blob.
...
def _upload_attempt_log(
    ...
    *,
    ...
    """Best-effort upload of a per-attempt log file to GCS; never raises."""
    try:
...
def _upload_metrics(
    ...
    *,
    ...
    """Upload metrics.json to GCS if it exists; return True on success."""
    mf = output_dir / "metrics.json"
    if not mf.is_file():
        return False
    try:
...
def _upload_sentinel(
    ...
    *,
    ...
    """Upload status.json sentinel to GCS; return True on success."""
    try:
...
# ---------------------------------------------------------------------------
# Core orchestration (injectable subprocess runner + gcs client for tests)
# ---------------------------------------------------------------------------
...
def _run_with_ladder(
    *,
    ...
    Args:
        ...
        bucket_name:        GCS bucket name.
        project:            GCP project ID (or None to infer from ADC).
        gcs_client:         Injectable Bucket-like client (None = build lazily).
...
# ---------------------------------------------------------------------------
# Bootstrap: pull code from GCS + pip install
# ---------------------------------------------------------------------------
...
def _bootstrap(
    *,
    ...
    """Pull code from GCS and pip install requirements.

    Args:
        code_blob_prefix: Blob prefix for the uploaded code tree.
        local_code_dir:   Where to materialise the code on disk.
        cache_root:       Shared cache mount root used for all caches.
        bucket_name:      GCS bucket name.
        project:          GCP project ID (or None to infer from ADC).
        gcs_client:       Injectable Bucket-like client.

    Returns:
        (success: bool, error_message: str)
    """
    try:
        logger.info("Downloading code from GCS prefix %s", code_blob_prefix)
        ...
        return False, f"GCS pull failed: {exc}"

    # Set up caches under the shared cache mount
...
def main(
    *,
    ...
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [gke-cell] %(levelname)s %(message)s",
    )
    ...
    project = os.environ.get("OPENRESEARCH_GCP_PROJECT") or None
    ...
    logger.info(
        "GKE cell entrypoint start: cell_id=%s bucket=%s project=%s",
        cell_id, bucket_name, project,
    )

    # -----------------------------------------------------------------------
    # Scratch directories (inside the container's writable layer)
    # -----------------------------------------------------------------------
    scratch = Path(tempfile.mkdtemp(prefix="gke_cell_"))
