...
**Goal.** Give the RLM root model a free, deterministic, intra-run orientation cache so it stops re-deriving the same paper facts through paid `rlm_query` / `llm_query` sub-calls within a single run. The map accumulates the structured outputs of the three orientation primitives (`understand_section`, `extract_hyperparameters`, `detect_environment`) into a small (~1.5 KB), bounded JSON artifact the root can read in one cheap primitive call.
