...
| Evidence gate **self-attest escape closed** (2026-05-30 review) | ✅ done | dropped `and not report.baseline_metrics` from `_apply_evidence_gate` — the strict predicate is now the SOLE gate condition, so self-attested `baseline_metrics` from a failed run no longer rescues a `partial`/`reproduced` verdict. **Cost (stated):** an honest tri-state run (`success=False` + real partial metrics) now downgrades to `failed`. Known residual: gate checks *a* success+metrics row exists, not that it's *tied to* `baseline_metrics`. Tests: `test_evidence_gate.py` (+3), fixture-fixes in `test_preserved_marker.py`/`test_report.py`/`test_fatal_backend_gating.py`/`rdr/test_controller.py` |
| Replay CI regression (review #5) | ✅ done | `tests/fixtures/evidence_gate_replay/cases.json` (real pb_784/083 leaks + closed escape + positive control) + `test_evidence_gate_replay.py`; promotes `scripts/replay_run.py` to a committed-fixture CI guard |
| **SDAR A/B real-run validation** (review #1–3) | ☐ operator-launch | runbook `docs/runbooks/2026-05-30-validation-first-sdar-ab.md` — core-only → +Phase8 → +Phase9, with metrics harvester + evidence-truth check. Costs RunPod/OpenAI credits; not auto-runnable |
