...
_Last updated: 2026-06-02. This doc is the canonical "what looks wrong but isn't" plus "what's actually broken" register for the OpenResearch / ReproLab stack. Add new entries here when you discover a confusing failure mode; remove entries when the underlying bug is fixed AND the fix has shipped to `main`._
...
| 26 | `paper_grounding_failed` false positive — root returns `datasets` as list of stringified dicts instead of plain name strings (BUG-NEW-046) | low (advisory only) | **fixed 2026-06-02** | `backend/agents/paper_grounding.py::assert_paper_grounded` — extracts `name` field from dict-repr strings before grounding check |
| 27 | `compute_scope_invalid` — root passes string for `compute_scope` because `environment_spec.json::compatibility_notes` says "CPU" (detect_environment runs locally, not on RunPod) (BUG-NEW-047) | low (train.py GPU-detects independently) | **fixed 2026-06-02** | `backend/agents/environment_detective.py` — compatibility_notes now explicitly states local CPU ≠ RunPod GPU |
| 28 | GEPA `plan_reproduction` produces 0 candidates — seed evaluation via claude-oauth (~2 min) exhausts the 60s timeout before any proposals are generated (BUG-NEW-048) | medium (GEPA ineffective on claude-oauth path) | **fixed 2026-06-02** | `backend/config.py` — `gepa_timeout_plan_s` raised 60→180s, `gepa_timeout_baseline_s` 30→90s, `gepa_timeout_improve_s` 60→180s |
| 29 | `RUNPOD_TRANSIENT_500` triggers same 3-retry→escalate path as `CAPACITY_EXHAUSTED`, wasting escalation slots on momentary API flakiness (BUG-NEW-049) | low-medium | **documented** 2026-06-02 — intentional design (3 retries already exhausted before escalation; 500 may be capacity under different marker); comment added to code |
| 30 | Root model passes empty `results={}` to `verify_against_rubric` → stale degraded cache hit returns `degraded_no_metrics` (0.0) despite successful training (BUG-NEW-050) | **HIGH** | **fully fixed 2026-06-02** (`a1377d0` + `33b90c1`) | `primitives.py::verify_against_rubric` fallback to `experiment_runs.jsonl`; `experiment_runs.jsonl` in cache key; system prompt explicit instruction to pass full `run_experiment` return value; anytime fallback scoring persists a score before timeout |
| 31 | Multi-experiment train.py exceeds wall-clock — CIFAR-10 MixUp + ERM + CIFAR-100 run ~6000s total; 3rd experiment times out; score=0 despite successful earlier experiments (BUG-NEW-051) | **HIGH** | **partially mitigated 2026-06-02** (`33b90c1`) — anytime scoring now captures rubric score after first successful experiment; full fix requires multi-fidelity scope planning (Proposal 2 in `2026-06-02-reliability-overhaul-design.md`) | workaround: `REPROLAB_BASELINE_EXTRA_GUIDANCE` to scope to CIFAR-10 only, or raise `--max-wall-clock` to 10800s |
