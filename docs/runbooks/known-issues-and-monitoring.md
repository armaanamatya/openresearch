...
| 30 | Root model passes empty `results={}` to `verify_against_rubric` → stale degraded cache hit returns `degraded_no_metrics` (0.0) despite successful training (BUG-NEW-050) | **HIGH** | **fixed 2026-06-02** | `backend/agents/rlm/primitives.py::verify_against_rubric` — (1) fallback to last successful `experiment_runs.jsonl` entry when results is empty; (2) `experiment_runs.jsonl` included in cache key to invalidate on new experiments |
| 31 | Multi-experiment train.py exceeds wall-clock — CIFAR-10 MixUp + ERM + CIFAR-100 run ~6000s total; 3rd experiment times out; `run_experiment` records `success: False`; BUG-NEW-050 fallback cannot recover (entry itself is failed) (BUG-NEW-051) | **HIGH** | **documented 2026-06-02** — workaround: add `REPROLAB_BASELINE_EXTRA_GUIDANCE` to scope to CIFAR-10 only, or raise `--max-wall-clock` to 10800s | `runs/<id>/experiment_runs.jsonl` shows `success: false, metrics: {}`; CIFAR-10 results visible in `exec.log` but not structured |
