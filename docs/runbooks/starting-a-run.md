...
Copy-paste guide to launch a ReproLab reproduction **and** confirm the whole
...
`ANTHROPIC_API_KEY` / `REPROLAB_RUNPOD_API_KEY`. Every launch is prefixed with
...
6. **RunPod credentials — ONLY for `--sandbox runpod`.** Two gates: an API key
   ...
   - FIX: get a key at <https://console.runpod.io/account/api-keys>, set
     `REPROLAB_RUNPOD_API_KEY=…` in `.env`. Create a key if absent:
     `ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519 -N ''` (or point
     `REPROLAB_RUNPOD_SSH_KEY_PATH` at an existing one). Preflight:
...
7. **Shell-shadows-.env guard** — your shell must NOT export the three keys, or
   ...
   - CHECK: `[ -z "$OPENAI_API_KEY$ANTHROPIC_API_KEY$REPROLAB_RUNPOD_API_KEY" ]`
   - FIX: `unset OPENAI_API_KEY ANTHROPIC_API_KEY REPROLAB_RUNPOD_API_KEY` for
...
( cd /Volumes/CS_Stuff/openresearch || exit 1
  ...
  [ -z "$OPENAI_API_KEY$ANTHROPIC_API_KEY$REPROLAB_RUNPOD_API_KEY" ] \
  ...
  # grep -Eq '^REPROLAB_RUNPOD_API_KEY=.+' .env \
  #   || { echo "NOT-READY: REPROLAB_RUNPOD_API_KEY missing in .env"; exit 1; }
  # test -f "${REPROLAB_RUNPOD_SSH_KEY_PATH:-$HOME/.ssh/id_ed25519}" \
...
CLI flag — it is passed as guidance via `REPROLAB_BASELINE_EXTRA_GUIDANCE` (pin
...
REPROLAB_BASELINE_EXTRA_GUIDANCE="Reproduce only the two smallest variants: Qwen3-1.7B and Qwen2.5-3B. Single GPU." \
...
cd /Volumes/CS_Stuff/openresearch && \
REPROLAB_BASELINE_EXTRA_GUIDANCE="Reproduce only the two smallest variants: Qwen3-1.7B and Qwen2.5-3B. Single GPU." \
env -u OPENAI_API_KEY -u ANTHROPIC_API_KEY -u REPROLAB_RUNPOD_API_KEY \
  .venv/bin/python -m backend.cli reproduce 2605.15155 \
...
| `RunPod API key is missing` / `401` at **pod creation** (before any pod work) | **RunPod auth** | `grep -Eq '^REPROLAB_RUNPOD_API_KEY=.+' .env` fails, or shell shadows it | Set `REPROLAB_RUNPOD_API_KEY` in `.env`; prefix launch with `env -u REPROLAB_RUNPOD_API_KEY`. |
| `SSH private key not found` at pod creation | **RunPod SSH** | `test -f "${REPROLAB_RUNPOD_SSH_KEY_PATH:-$HOME/.ssh/id_ed25519}"` fails | `ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519 -N ''` or set `REPROLAB_RUNPOD_SSH_KEY_PATH`. |
| Run routes to wrong provider / unexpected `401` while `.env` looks right | **Shell shadows .env (BUG-LR-014)** | `env \| grep -E 'OPENAI_API_KEY\|ANTHROPIC_API_KEY\|REPROLAB_RUNPOD_API_KEY'` shows an export | Launch with `env -u …` prefix, or `unset` the vars for the session. |
...
- **Lossy paper text proceeds by default.** `REPROLAB_ALLOW_LOSSY_PAPER_TEXT`
