...
- **Real durable-controller submit** (`backend/agents/rlm/{controller_cluster,controller_launch,controller_entry}.py`, `backend/services/events/live_runs.py`): replaces the `NotImplementedError` stub. Takeover-safe ordering (acquire → is_current → submit → wait_ready → reap → handle); local fallback only when no remote Job is live (confirmed-deleted not-ready → Popen; unconfirmable → `ControllerStuck`, fail loud, never split-brain; contended → adopt). `LiveRunState` gains a `ControllerHandle` so a `pid=None` durable run is active. In-Pod lease heartbeat + `sweep_orphaned_controllers` (distinct owner, expired-only — never takes over a healthy controller). `GcsK8sControllerCluster` is the drill-exercised real impl; the pure ordering/sweep logic is fully hermetically tested.
...
- New flags: `OPENRESEARCH_{CPU_CLOUD_CELLS, CPU_POOL_LABEL, CONTROLLER_READY_TIMEOUT_S, CONTROLLER_BACKOFF_LIMIT, CELL_FENCE_EPOCH, DEMO_STATUS_TERMINAL_GUARD}` + real `OPENRESEARCH_DURABLE_CONTROLLER`. New `run_warning` codes: `cpu_cloud_fallback`, `durable_controller_fallback`.
