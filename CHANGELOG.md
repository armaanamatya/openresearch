...
### Changed (Cross-cloud durable execution hardening — 2026-07-17)
- Durable campaign controllers now run end-to-end inside GKE or AKS: staged PDF/run-spec inputs, cloud-specific generation/ETag CAS leases, a unique per-launch owner preserved across pod retries, in-pod heartbeat and stale-fence reaping, RWX campaign state, and object-store status checkpoints.
- Durable mode now fails closed instead of falling back to the initiating laptop. Ambiguous Job creates are confirmed deleted or raised as `ControllerStuck`, and readiness requires a Running/Succeeded pod rather than `Job.status.active`.
- Controller launches require a pinned full orchestrator image plus explicit LLM-dollar, GPU-dollar, and GPU-hour caps. Intentional pause/money-halt exits do not trigger Kubernetes backoff retries; genuine crashes still do.
- Durable campaigns preserve the submitted root model, execution/GPU modes, and compute-minimization policy across controller and attempt boundaries. Autonomous `opus-foundry` credentials now come from the cloud CSI store while only non-secret Foundry routing enters the Job spec.
- CPU Jobs now default to the system-node labels actually provisioned by GKE (`reprolab/node-type=system`) and AKS (`kubernetes.azure.com/mode=system`). Autonomous mode defaults to GCP but preserves an explicit Azure target.
- Added the cross-cloud design and operator runbook, including a heterogeneous ML/computational-biology pilot and an explicit wet-lab scope boundary.

### Added (Cloud-native durable controller + CPU cloud lane — 2026-07-12; spec `docs/history/specs/2026-07-12-cloud-native-durable-and-cpu-lane-design.md`, handoff `docs/runbooks/2026-07-12-durable-cpu-lane-session-handoff.md`)
All flag-gated **default-OFF, byte-identical off**; "on for gcp" is set by the deployment env (Option Y). Registry stays **19** primitives.
- **Renew-invariant `fence_epoch`** (`backend/services/runtime/blob_lease.py`): a stable fence token distinct from the CAS generation (which `renew()` advances every heartbeat), plus `reap_stale_fence_epochs`. Fixes the self-reaping bug where a controller would delete its own still-running Jobs after its first lease renewal. `reap_older_generations` left intact.
- **Real durable-controller submit** (`backend/agents/rlm/{controller_cluster,controller_launch,controller_entry}.py`, `backend/services/events/live_runs.py`): replaces the `NotImplementedError` stub. The 2026-07-17 hardening above supersedes this entry's original GCP-only/fallback behavior.
- **CPU cloud lane** (`backend/agents/rlm/cpu_class.py`, `k8s_job_cell_runner.py`): deterministic classifier (hard GPU signals override a soft `accelerator=cpu`; unknown ⇒ GPU); `_build_job_manifest` gains a CPU branch (no `nvidia.com/gpu`, CPU nodeSelector, cpu/mem requests) with the GPU path byte-identical (golden test); `run_matrix` routes CPU-class cells to CPU Jobs and, on all-cells cluster infra-failure, reruns them in-process via `gpu_cell_runner.run_matrix` (dict-based detection) with a `cpu_cloud_fallback` warning. Gated on `OPENRESEARCH_CPU_CLOUD_CELLS`.
- **WS1-H1 demo_status terminal guard** (`backend/agents/rlm/run.py`): the cost-summary daemon re-reads the live status before its atomic replace and skips a stale non-terminal republish over a terminal write. Gated on `OPENRESEARCH_DEMO_STATUS_TERMINAL_GUARD`; verdict-surface-protective.
- New flags: `OPENRESEARCH_{CPU_CLOUD_CELLS, CPU_POOL_LABEL, CONTROLLER_READY_TIMEOUT_S, CONTROLLER_BACKOFF_LIMIT, CELL_FENCE_EPOCH, DEMO_STATUS_TERMINAL_GUARD}` + real `OPENRESEARCH_DURABLE_CONTROLLER`. New `run_warning` code: `cpu_cloud_fallback`.
- **Deferred, not done (verdict red line):** WS-Ext (`result_fidelity` per-claim `kind` measurement) is deliberately not shipped — it changes the deterministic verdict and needs its own frozen-Adam A/B. Corrected anchors + path in the handoff.

### Added (OAuth-root degenerate-loop detector + harness backstop — plan `docs/history/plans/2026-06-17-oauth-root-reliability-and-harness-backstop.md`)
...
### Added (BES conversion + archival correctness — P0 prerequisite; all flag-gated default-OFF, parity-preserving; spec `docs/history/specs/2026-06-17-bes-evidence-first-and-conversion-remediation-design.md`, plan `docs/history/plans/2026-06-17-bes-conversion-archival-correctness.md`)
...
- All guards default-OFF (byte-for-byte unchanged until enabled). Decision: the non-Claude executor
  ...
  recommended validated executor. Plan: `docs/history/plans/2026-06-19-gptchat-rlm-root-optimization.md`.
...
- **`rdr` — rubric-driven paper-reproduction harness (`--mode rdr`).**
  ...
  - Design spec: `docs/history/specs/2026-05-22-rubric-driven-harness-design.md`.
